...
describe("o envelope carrega a thread do provider", () => {
  ...
  it("o handler LÊ a coluna e a PASSA ao adapter — o elo que some sem barulho", () => {
    ...
    // CINCO desde que o envio por URL de mídia (GIF) passou a sair: texto,
    // mídia storage, mídia URL, modelo e contato.
    const passagens = [...fonte.matchAll(/providerConversationId:\s*c\.provider_conversation_id/g)];
    expect(
      passagens.length,
      "todos os call sites (texto, mídia storage, mídia URL, modelo e contato) precisam passar",
    ).toBe(5);
  });
});
