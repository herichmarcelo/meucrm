  ...
  try {
    ...
    if (!run.is_dry_run && run.conversation_id) {
      const { data: convRaw } = await admin
        .from("conversations")
        .select(
          `id, group_chat_id, is_group, contacts:contact_id(phone_number, wa_identity, wa_lid, email), channel_sessions:channel_session_id(${CHANNEL_SESSION_REF_COLUMNS})`,
        )
      ...
      const conv = convRaw as unknown as {
        ...
        is_group: boolean;
        contacts: { phone_number: string | null; wa_identity: string | null; wa_lid: string | null; email: string | null } | null;
